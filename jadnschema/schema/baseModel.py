...
class BaseModel(pydanticBase):
    ...
    def value(self, *, include: UntStrMapUnion = None, exclude: UntStrMapUnion = None, by_alias: bool = True, skip_defaults: bool = None, exclude_unset: bool = True, exclude_defaults: bool = False, exclude_none: bool = False) -> Any:  # pylint: disable=W0221
        ...
        """
        data = self.model_dump(include=include, exclude=exclude, by_alias=by_alias, skip_defaults=skip_defaults, exclude_unset=exclude_unset, exclude_defaults=exclude_defaults, exclude_none=exclude_none)
        if pydantic.RootModel in data:
            return data[pydantic.RootModel]
        return data
    ...
    def keys(self) -> Tuple[str, ...]:
        ...
        """
        return tuple(attr for attr in self.model_fields if self[attr])
    ...
    def items(self) -> Tuple[Tuple[str, Any], ...]:
        ...
        """
        return tuple(idx for idx in self.model_dump(by_alias=True, exclude_unset=True).items())
    ...
    def values(self) -> Tuple[Any, ...]:
        ...
        """
        d = self.model_dump(by_alias=True, exclude_unset=True)
        return tuple(d)
